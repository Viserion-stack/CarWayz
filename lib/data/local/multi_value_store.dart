...
import 'package:car_wayz/data/local/adapter/storage_adapter.dart';
import 'package:car_wayz/data/local/storable.dart';
...
class MultiValueStore<T extends Storable> {
  ...
  Future<bool> add({required T value, bool prepend = false}) async {
    ...
    final updatedValues =
        prepend ? previousValues.prepend(value) : previousValues.append(value);
  ...
  Future<bool> addAll(
      {required Iterable<T> values, bool prepend = false}) async {
    final previousValues = await getAll();
    final newValues = _allowDuplicates
        ? values
        : values.where((element) => !previousValues.contains(element));
    if (!_allowDuplicates && newValues.isEmpty) {
      return false;
    }
    final updatedValues = prepend
  ...
  Future<void> _write(Iterable<T> iterable) => _adapter.write(
        key: _key,
        value: jsonEncode(
            iterable.map((value) => value.toJson()).toList(growable: false)),
      );
}
