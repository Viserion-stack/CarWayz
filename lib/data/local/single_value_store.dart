import 'dart:convert';
import 'package:car_wayz/core/fpdarts.dart';
...
class SingleValueStore<T extends Storable> {
  ...
  TaskEither<SingleValueStoreFailure, Unit> write({required T value}) =>
      tryCatchE<SingleValueStoreFailure, Unit>(
        () async {
  ...
  TaskEither<SingleValueStoreFailure, T> read() =>
      tryCatchE<SingleValueStoreFailure, T>(
        () async {
  ...
  TaskEither<SingleValueStoreFailure, Unit> delete() =>
      tryCatchE<SingleValueStoreFailure, Unit>(
        () async {
